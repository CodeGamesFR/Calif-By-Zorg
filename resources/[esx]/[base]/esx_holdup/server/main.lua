...
AddEventHandler('esx_holdup:toofar', function(robb)
	...
	for i = 1, #xPlayers, 1 do
		...
		if xPlayer and xPlayer.job.name == 'police' then
			TriggerClientEvent('esx:showNotification', xPlayers[i], _U('robbery_cancelled_at', Config.Stores[robb].storeName))
	...
	if robbers[_source] then
		...
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Config.Stores[robb].storeName))
	end
end)
...
AddEventHandler('esx_holdup:rob', function(robb)
	...
	if Config.Stores[robb] then
		if (os.time() - Config.Stores[robb].lastRobbed) < Config.TimerBeforeNewRob and Config.Stores[robb].lastRobbed ~= 0 then
			TriggerClientEvent('esx:showNotification', _source, _U('recently_robbed', Config.TimerBeforeNewRob - (os.time() - Config.Stores[robb].lastRobbed)))
			return
		end
		...
		if not rob then
			if cops >= Config.PoliceNumberRequired then
				...
				for i = 1, #xPlayers, 1 do
					...
					if xPlayer and xPlayer.job.name == 'police' then
						TriggerClientEvent('esx:showNotification', xPlayers[i], _U('rob_in_prog', Config.Stores[robb].storeName))
						TriggerClientEvent('esx_holdup:setblip', xPlayers[i], Config.Stores[robb].coords)
					end
				end

				TriggerClientEvent('esx:showNotification', _source, _U('started_to_rob', Config.Stores[robb].storeName))
					...
					if robbers[_source] then
						...
						if xPlayer then
							...
							for i = 1, #xPlayers, 1 do
								...
								if xPlayer and xPlayer.job.name == 'police' then
									TriggerClientEvent('esx:showNotification', xPlayers[i], _U('robbery_complete_at', Config.Stores[robb].storeName))
									TriggerClientEvent('esx_holdup:killblip', xPlayers[i])
								end
							end
						end
