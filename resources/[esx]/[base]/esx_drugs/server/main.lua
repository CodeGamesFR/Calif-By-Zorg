...
local function HarvestWeed(xPlayer)
	if CopsConnected < Config.RequiredCopsWeed then
		TriggerClientEvent('esx:showNotification', xPlayer.source, _U('act_imp_police', CopsConnected, Config.RequiredCopsWeed))
		return
	end
	...
	SetTimeout(Config.TimeToFarm, function()
		if PlayersHarvestingWeed[xPlayer.source] then
			if xPlayer.canCarryItem('weed', 1) then
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('inv_full_weed'))
			end
		end
	end)
end
	...
	SetTimeout(Config.TimeToProcess, function()
		if PlayersTransformingWeed[xPlayer.source] then
			...
			if poochQuantity > 35 then
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('too_many_pouches'))
			elseif weedQuantity < 5 then
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('not_enough_weed'))
			else
...
AddEventHandler('esx_drugs:startTransformWeed', function()
	...
	TriggerClientEvent('esx:showNotification', source, _U('packing_in_prog'))
	...
	SetTimeout(Config.TimeToSell, function()
		if PlayersSellingWeed[xPlayer.source] then
			...
			if poochQuantity == 0 then
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('no_pouches_sale'))
			else
				xPlayer.removeInventoryItem('weed_pooch', 1)

				if CopsConnected == 0 then
					xPlayer.addAccountMoney('dirtycash', 250)
					TriggerClientEvent('esx:showNotification', xPlayer.source, _U('sold_one_weed'))
...
local function HarvestCoke(xPlayer)
	if CopsConnected < Config.RequiredCopsCoke then
		TriggerClientEvent('esx:showNotification', xPlayer.source, _U('act_imp_police', CopsConnected, Config.RequiredCopsCoke))
		return
	end
	...
	SetTimeout(Config.TimeToFarm, function()
		if PlayersHarvestingCoke[xPlayer.source] then
			if xPlayer.canCarryItem('coke', 1) then
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('inv_full_coke'))
			end
		end
	end)
end
	...
	SetTimeout(Config.TimeToProcess, function()
		if PlayersTransformingCoke[xPlayer.source] then
			...
			elseif cokeQuantity < 5 then
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('not_enough_coke'))
			else
		...
		if PlayersSellingCoke[xPlayer.source] then
			...
			if poochQuantity == 0 then
				...
				if CopsConnected == 0 then
					...
					TriggerClientEvent('esx:showNotification', xPlayer.source, _U('sold_one_coke'))
				elseif CopsConnected == 1 then
					xPlayer.addAccountMoney('dirtycash', 400)
					TriggerClientEvent('esx:showNotification', xPlayer.source, _U('sold_one_coke'))
				elseif CopsConnected == 2 then
...
local function HarvestMeth(xPlayer)
	if CopsConnected < Config.RequiredCopsMeth then
		TriggerClientEvent('esx:showNotification', xPlayer.source, _U('act_imp_police', CopsConnected, Config.RequiredCopsMeth))
		return
	end
	...
	SetTimeout(Config.TimeToFarm, function()
		if PlayersHarvestingMeth[xPlayer.source] then
			if xPlayer.canCarryItem('meth', 1) then
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('inv_full_meth'))
			end
		end
	end)
end
	...
	SetTimeout(Config.TimeToProcess, function()
		if PlayersTransformingMeth[xPlayer.source] then
			...
			elseif methQuantity < 5 then
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('not_enough_meth'))
			else
		...
		if PlayersSellingMeth[xPlayer.source] then
			...
			if poochQuantity == 0 then
				...
				if CopsConnected == 0 then
					...
					TriggerClientEvent('esx:showNotification', xPlayer.source, _U('sold_one_meth'))
				elseif CopsConnected == 1 then
					xPlayer.addAccountMoney('dirtycash', 450)
					TriggerClientEvent('esx:showNotification', xPlayer.source, _U('sold_one_meth'))
				elseif CopsConnected == 2 then
...
local function HarvestOpium(xPlayer)
	if CopsConnected < Config.RequiredCopsOpium then
		TriggerClientEvent('esx:showNotification', xPlayer.source, _U('act_imp_police', CopsConnected, Config.RequiredCopsOpium))
		return
	end
	...
	SetTimeout(Config.TimeToFarm, function()
		if PlayersHarvestingOpium[xPlayer.source] then
			if xPlayer.canCarryItem('opium', 1) then
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('inv_full_opium'))
			end
		end
	end)
end
	...
	SetTimeout(Config.TimeToProcess, function()
		if PlayersTransformingOpium[xPlayer.source] then
			...
			elseif opiumQuantity < 5 then
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('not_enough_opium'))
			else
		...
		if PlayersSellingOpium[xPlayer.source] then
			...
			if poochQuantity == 0 then
				...
				if CopsConnected == 0 then
					...
					TriggerClientEvent('esx:showNotification', xPlayer.source, _U('sold_one_opium'))
				elseif CopsConnected == 1 then
					xPlayer.addAccountMoney('dirtycash', 500)
					TriggerClientEvent('esx:showNotification', xPlayer.source, _U('sold_one_opium'))
				elseif CopsConnected == 2 then
...
ESX.RegisterUsableItem('weed', function(source)
	...
	TriggerClientEvent('esx_drugs:onPot', xPlayer.source)
	TriggerClientEvent('esx:showNotification', xPlayer.source, _U('used_one_weed'))
end)