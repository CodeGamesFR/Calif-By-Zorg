...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('esx_vehicleshop:getCommercialVehicles', function(vehicles)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			_TriggerServerEvent('esx_vehicleshop:returnProvider', data.current.value)
			Citizen.Wait(300)
			menu.close()
									...
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones[LastZone[1]].ShopOutside.Pos, Config.Zones[LastZone[1]].ShopOutside.Heading, function(vehicle)
											...
											if Config.EnableOwnedVehicles then
												_TriggerServerEvent('esx_vehicleshop:setVehicleOwned', vehicleProps, getVehicleType(vehicleProps.model))
											end
								...
								ESX.TriggerServerCallback('esx_vehicleshop:buyVehicleSociety', function(hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones[LastZone[1]].ShopOutside.Pos, Config.Zones[LastZone[1]].ShopOutside.Heading, function(vehicle)
											...
											_TriggerServerEvent('esx_vehicleshop:setVehicleOwnedSociety', playerData.job.name, vehicleProps, getVehicleType(vehicleProps.model))
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_' .. ESX.PlayerData.job.name, _U('car_dealer'), tonumber(data2.value))
					end
				end
			end, function(data2, menu2)
				menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				_TriggerServerEvent('esx_vehicleshop:sellVehicle', model)
				_TriggerServerEvent('esx_vehicleshop:addToList', GetPlayerServerId(closestPlayer), model, newPlate, LastZone[1])

				if Config.EnableOwnedVehicles then
					_TriggerServerEvent('esx_vehicleshop:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, getVehicleType(vehicleProps.model))
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				ESX.TriggerServerCallback('esx:getOtherPlayerData', function(xPlayer)
					...
					if Config.EnableSocietyOwnedVehicles then
						_TriggerServerEvent('esx_vehicleshop:setVehicleOwnedSociety', xPlayer.job.name, vehicleProps, getVehicleType(vehicleProps.model))
						ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
					else
						ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
					end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						vehicleProps.plate = newPlate
						SetVehicleNumberPlateText(LastVehicles[#LastVehicles], newPlate)
						_TriggerServerEvent('esx_vehicleshop:rentVehicle', model, vehicleProps.plate, GetPlayerName(closestPlayer), CurrentVehicleData.price, amount, GetPlayerServerId(closestPlayer), LastZone[1])
						...
						_TriggerServerEvent('esx_vehicleshop:setVehicleForAllPlayers', vehicleProps, Config.Zones[LastZone[1]].ShopInside.Pos, 5.0)
					end
				end
			end, function(data2, menu2)
				menu2.close()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('esx_vehicleshop:getStockItem', LastZone[1], itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('esx_vehicleshop:putStockItems', LastZone[1], itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
