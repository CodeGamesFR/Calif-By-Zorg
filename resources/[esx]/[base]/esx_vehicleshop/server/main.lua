...
for k, v in pairs(Config.Society) do
	TriggerEvent('esx_phone:registerNumber', k, _U('dealer_customers'), false, false)
...
AddEventHandler('esx_vehicleshop:getStockItem', function(job, itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_' .. job, function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('not_enough_in_society'))
		end
	end)
end)
...
AddEventHandler('esx_vehicleshop:putStockItems', function(job, itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_' .. job, function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('have_deposited', count, ESX.GetItem(itemName).label))
		else
