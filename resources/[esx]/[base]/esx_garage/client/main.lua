...
AddEventHandler('esx_garage:hasEnteredMarker', function(name, part, parking)
	...
	if part == 'Parking' then
		...
		if IsPedInAnyVehicle(playerPed, false) and not IsAnyVehicleNearPoint(parkingPos, 1.0) then
			...
			_TriggerServerEvent('esx_garage:setParking', name, parking, vehicleProps)

			if Config.EnableOwnedVehicles then
				_TriggerServerEvent('esx_garage:updateOwnedVehicle', vehicleProps)
			end
		end
	end
end)
...
AddEventHandler('esx_property:hasExitedMarker', function(name, part, parking)
	if part == 'Parking' then
		...
		if IsPedInAnyVehicle(playerPed, false) and not IsAnyVehicleNearPoint(parkingPos, 1.0) then
			_TriggerServerEvent('esx_garage:setParking', name, parking, false)
		end
	end
end)
