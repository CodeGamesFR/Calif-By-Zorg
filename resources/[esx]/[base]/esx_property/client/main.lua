...
function EnterProperty(name, owner)
	...
	_TriggerServerEvent('esx_property:saveLastProperty', name)
...
function ExitProperty(name)
	...
	_TriggerServerEvent('esx_property:deleteLastProperty')
...
function OpenPropertyMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property', {
		...
		elseif data.current.value == 'leave' then
			_TriggerServerEvent('esx_property:removeOwnedProperty', property.name)
		elseif data.current.value == 'buy' then
			_TriggerServerEvent('esx_property:buyProperty', property.name)
		elseif data.current.value == 'rent' then
			_TriggerServerEvent('esx_property:rentProperty', property.name)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties_actions', {
			...
			elseif data2.current.value == 'leave' then
				_TriggerServerEvent('esx_property:removeOwnedProperty', data.current.value)
			end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties_actions', {
			...
			if data2.current.value == 'buy' then
				_TriggerServerEvent('esx_property:buyProperty', data.current.value)
			elseif data2.current.value == 'rent' then
				_TriggerServerEvent('esx_property:rentProperty', data.current.value)
		...
		elseif data.current.value == 'remove_cloth' then
			ESX.TriggerServerCallback('esx_property:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
					...
					menu2.close()
					_TriggerServerEvent('esx_property:removeOutfit', data2.current.value)
	...
	ESX.TriggerServerCallback('esx_property:getPropertyInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				_TriggerServerEvent('esx_property:getItem', owner, data.current.type, data.current.value, data.current.ammo)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
					...
					if quantity == nil then
						...
						_TriggerServerEvent('esx_property:getItem', owner, data.current.type, data.current.value, quantity)
						ESX.SetTimeout(300, function()
							OpenRoomInventoryMenu(property, owner)
						end)
					end
	...
	ESX.TriggerServerCallback('esx_property:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				_TriggerServerEvent('esx_property:putItem', owner, data.current.type, data.current.value, data.current.ammo)
				ESX.SetTimeout(300, function()
					OpenPlayerInventoryMenu(property, owner)
				end)
			else
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
					...
					if quantity == nil then
						...
						_TriggerServerEvent('esx_property:putItem', owner, data.current.type, data.current.value, tonumber(data2.value))
						ESX.SetTimeout(300, function()
							OpenPlayerInventoryMenu(property, owner)
						end)
					end
