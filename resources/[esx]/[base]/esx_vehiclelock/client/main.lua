			...
			if data2.current.value ~= nil then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mykey', {
							...
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
								...
								if data4.current.value == 'yes' then
									if distance ~= -1 and distance <= 3.0 then
										_TriggerServerEvent('esx_vehiclelock:changeowner', GetPlayerServerId(player), vehPlate, vehicleProps)
										ESX.UI.Menu.CloseAll()
									else
					...
					if data3.current.value == 'preterkey' then
						if distance ~= -1 and distance <= 3.0 then 
							_TriggerServerEvent('esx_vehiclelock:preterkey', GetPlayerServerId(player), data2.current.value)
							ESX.UI.Menu.CloseAll()
						else
							ESX.ShowNotification("Aucun joueur à proximité")
						end
					...
					if data3.current.value == 'jeterkey' then
						_TriggerServerEvent('esx_vehiclelock:deletekey', data2.current.value)
						ESX.UI.Menu.CloseAll()
					end
			...
			ESX.TriggerServerCallback('esx_vehiclelock:getVehiclesnokey', function(Vehicles2)
				...
				if Vehicles2 == nil then
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'backey', {
						...
						menu2.close()
						_TriggerServerEvent('esx_vehiclelock:registerkey', data2.current.value, 'no')
					end, function(data2, menu2)
					end)
				end
			end)
