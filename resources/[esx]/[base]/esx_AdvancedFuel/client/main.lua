...
Citizen.CreateThread(function()
	_TriggerServerEvent('essence:addPlayer')
...
Citizen.CreateThread(function()
	...
	while true do
				...
				if not (IsPedInAnyHeli(playerPed) or IsPedInAnyPlane(playerPed) or IsPedInAnyBoat(playerPed)) then
					...
					if (menu) then
						...
						TriggerEvent('GUI:Option', settings[lang].confirm, function(cb)
							if (cb) then
								menu = not menu
								_TriggerServerEvent('essence:buy', int, stationNumber, false)
							end
						end)
				...
				if not (isBlackListedModel(vehModelName) or IsPedInAnyHeli(playerPed) or IsPedInAnyPlane(playerPed)) then
					...
					if (menu) then
						...
						TriggerEvent('GUI:Option', settings[lang].confirm, function(cb)
							if (cb) then
								menu = not menu
								_TriggerServerEvent('essence:buy', int, stationBoatNumber, false)
							end
						end)
				...
				if not (isBlackListedModel(vehModelName) or IsPedInAnyHeli(playerPed) or IsPedInAnyBoat(playerPed)) then
					...
					if (menu) then
						...
						TriggerEvent('GUI:Option', settings[lang].confirm, function(cb)
							if (cb) then
								menu = not menu
								_TriggerServerEvent('essence:buy', int, stationPlaneNumber, false)
							else
				...
				if not (isBlackListedModel(vehModelName) or IsPedInAnyPlane(playerPed) or IsPedInAnyBoat(playerPed)) then
					...
					if (menu) then
						...
						TriggerEvent('GUI:Option', settings[lang].confirm, function(cb)
							if (cb) then
								menu = not menu
								_TriggerServerEvent('essence:buy', int, stationHeliNumber, false)
							end
						end)
		...
		elseif IsPedOnFoot(playerPed) and (nearStation or nearPlaneStation or nearHeliStation or nearBoatStation) then
			...
			if IsControlJustPressed(0, 38) then
				_TriggerServerEvent('essence:buyCan')
			end
		else
			Citizen.Wait(200)
		end
...
AddEventHandler('essence:setEssence', function(es, plate)
...
AddEventHandler('advancedFuel:setEssence', function(percent, plate)
