...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_shops:requestDBItems', function(dbItems)
		shopItems = dbItems
	end)
end)
...
RegisterNetEvent('esx_shops:rob')
						...
						while true do
							...
							if DoesEntityExist(object) then
								if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(object)) <= 1.5 then
									...
									_TriggerServerEvent('esx_shops:pickUp', store)
									break
								end
							else
								break
...
RegisterNetEvent('esx_shops:msgPolice')
AddEventHandler('esx_shops:msgPolice', function(store)
	...
	while true do
		...
		for i = 1, #Config.Zones, 1 do
			if Config.Zones[i].ped.handle and IsPedDeadOrDying(Config.Zones[i].ped.handle) then
				_TriggerServerEvent('esx_shops:pedDead', i)
			end
		end
	end
end)
			...
			for i = 1, #Config.Zones do
				if Config.Zones[i].ped.handle then
					...
					if HasEntityClearLosToEntityInFront(plyPed, shopPed, 19) and not IsPedDeadOrDying(shopPed) and #(plyCoords - GetEntityCoords(shopPed)) <= 5.0 then
						if not robbing then
							ESX.TriggerServerCallback('esx_shops:canRob', function(enoughCops, canRob)
								if enoughCops and canRob then
									...
									if #(GetEntityCoords(plyPed) - GetEntityCoords(shopPed)) <= 7.5 then
										if not IsPedDeadOrDying(shopPed) then
											_TriggerServerEvent('esx_shops:rob', i)
								...
								elseif enoughCops and not canRob then
									TriggerEvent('esx_shops:talk', i, 'Je me suis déjà fais volé la caisse est vide', 5)
									Citizen.Wait(2500)
	...
	RageUI.IsVisible(RMenu.Get('rageui', 'shop'), true, false, true, function()
		for i = 1, #shopItems, 1 do
			RageUI.Button(shopItems[i].label, '', {RightLabel = ('%s$'):format(shopItems[i].price)}, true, function(Hovered, Active, Selected)
				if Selected then
					_TriggerServerEvent('esx_shops:buyItem', shopItems[i].value, 1, CurrentZone)
				end
			end)
		end
	end, function()
