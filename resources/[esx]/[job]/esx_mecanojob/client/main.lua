			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						_TriggerServerEvent('esx_society:removeVehicleFromGarage', 'mecano', vehicleProps)
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mecano_harvest', {
			...
			if data.current.value == 'gaz_bottle' then
				menu.close()
				_TriggerServerEvent('esx_mecanojob:startHarvest')
			elseif data.current.value == 'fix_tool' then
				menu.close()
				_TriggerServerEvent('esx_mecanojob:startHarvest2')
			elseif data.current.value == 'caro_tool' then
				menu.close()
				_TriggerServerEvent('esx_mecanojob:startHarvest3')
			end
		end, function(data, menu)
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mecano_craft', {
			...
			if data.current.value == 'blow_pipe' then
				menu.close()
				_TriggerServerEvent('esx_mecanojob:startCraft')
			elseif data.current.value == 'fix_kit' then
				menu.close()
				_TriggerServerEvent('esx_mecanojob:startCraft2')
			elseif data.current.value == 'caro_kit' then
				menu.close()
				_TriggerServerEvent('esx_mecanojob:startCraft3')
			end
		end, function(data, menu)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
					...
					if var == nil or var < 0 then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							menu.close()
							_TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mecano', _U('mechanic'), var)
						end
					end
				end, function(data, menu)
					menu.close()
						...
						if NPCOnJob then
							if NPCTargetDeleterZone then
								...
								if CurrentlyTowedVehicle == NPCTargetTowable then
									...
									_TriggerServerEvent('esx_mecanojob:onNPCJobMissionCompleted')
									StopNPCJob()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('esx_mecanojob:getStockItem', itemName, count)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					_TriggerServerEvent('esx_mecanojob:putStockItems', itemName, count)
...
AddEventHandler('esx_mecanojob:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		_TriggerServerEvent('esx_mecanojob:stopCraft')
		_TriggerServerEvent('esx_mecanojob:stopCraft2')
		_TriggerServerEvent('esx_mecanojob:stopCraft3')
	elseif zone == 'Garage' then
		_TriggerServerEvent('esx_mecanojob:stopHarvest')
		_TriggerServerEvent('esx_mecanojob:stopHarvest2')
		_TriggerServerEvent('esx_mecanojob:stopHarvest3')
	end
			...
			if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('esx_society:putVehicleInGarage', 'mecano', vehicleProps)
					else
						if
						...
						then
							_TriggerServerEvent('esx_service:disableService', 'mecano')
						end
					end
