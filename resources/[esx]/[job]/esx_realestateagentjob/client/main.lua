...
function OpenRealestateAgentMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'realestateagent', {
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data, menu) end)
		end
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell_property_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							_TriggerServerEvent('esx_realestateagentjob:sell', GetPlayerServerId(closestPlayer), data.data.name, amount)
							menu2.close()
						end
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'rent_property_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							_TriggerServerEvent('esx_realestateagentjob:rent', GetPlayerServerId(closestPlayer), data.data.name, amount)
							menu2.close()
						end
	...
	ESX.TriggerServerCallback('esx_realestateagentjob:getCustomers', function(customers)
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'customers', elements, function(data, menu)
			if data.value == 'revoke' then
				_TriggerServerEvent('esx_realestateagentjob:revoke', data.data.propertyName, data.data.propertyOwner)
				OpenCustomersMenu()
			elseif data.value == 'gps' then
