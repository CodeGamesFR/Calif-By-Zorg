...
function KeyboardInput()
	...
	if UpdateOnscreenKeyboard() ~= 2 then
		_TriggerServerEvent('esx_journalist:annonce', GetOnscreenKeyboardResult())
		Citizen.Wait(500)
		return
	else
		Citizen.Wait(500)
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_journalist', 'Journaliste', amount)
					end
				end
			end, function(data, menu)
				menu.close()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					OpenGetStocksMenu()
					_TriggerServerEvent('esx_journalist:getStockItem', data.current.value, count)
				end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					OpenPutStocksMenu()
					_TriggerServerEvent('esx_journalist:putStockItems', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('esx_society:removeVehicleFromGarage', 'journalist', vehicleProps)
			end, function(data, menu)
			...
			if IsControlJustReleased(0, 38) and IsJobTrue() then
				...
				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('esx_society:putVehicleInGarage', 'journalist', vehicleProps)
					else
						if
							GetEntityModel(vehicle) == `rumpo`
						then
							_TriggerServerEvent('esx_service:disableService', 'journalist')
						end
					end
...
RegisterNetEvent('esx_journalist:annonce')
AddEventHandler('esx_journalist:annonce', function(show, result)
	if show then
		newsText = result
		news = true
	else
