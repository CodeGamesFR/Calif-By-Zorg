...
AddEventHandler('esx_ambulancejob:removeItem', function(item)
	...
	if xPlayer.job.name == 'ambulance' and (item == 'medikit' or item == 'bandage') then
		...
		TriggerClientEvent('esx:showNotification', xPlayer.source, _U('used_%s'):format(item))
	end
end)
...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName)
	...
	if itemAvailable and xPlayer.job.name == 'ambulance' then
		if xPlayer.canCarryItem(itemName, 1) then
			...
			TriggerClientEvent('esx:showNotification', xPlayer.source, _U('max_item'))
		end
	end
end)
...
ESX.RegisterUsableItem('medikit', function(source)
	...
	TriggerClientEvent('esx:showNotification', xPlayer.source, _U('used_medikit'))
end)
...
ESX.RegisterUsableItem('bandage', function(source)
	...
	TriggerClientEvent('esx:showNotification', xPlayer.source, _U('used_bandage'))
end)
...
AddEventHandler('esx_ambulancejob:firstSpawn', function()
	...
	MySQL.Async.fetchScalar('SELECT isDead FROM users WHERE identifier = @identifier', {
		...
		if isDead == 1 then
			...
			TriggerClientEvent('esx_ambulancejob:requestDeath', xPlayer.source)
		end
	end)
end)
