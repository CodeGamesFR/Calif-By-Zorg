...
function SendDistressSignal()
	...
	_TriggerServerEvent('esx_phone:send', 'ambulance', _U('distress_message'), false, {
		x = coords.x,
		y = coords.y,
		z = coords.z
	})
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('esx_society:removeVehicleFromGarage', 'ambulance', vehicleProps)
			end, function(data, menu)
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		_TriggerServerEvent('esx_ambulancejob:giveItem', data.current.value)
	end, function(data, menu)
...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		_TriggerServerEvent('esx_ambulancejob:firstSpawn')
		FirstSpawn = false
	end
end)
...
AddEventHandler('esx:onPlayerDeath', function()
	isDead = true
	_TriggerServerEvent('esx_ambulancejob:setDeathStatus', 1)
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'ambulance' then
				...
				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('esx_society:putVehicleInGarage', 'ambulance', vehicleProps)
					end
