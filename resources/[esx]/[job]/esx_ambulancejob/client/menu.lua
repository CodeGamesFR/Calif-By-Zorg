					...
					ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(qtty)
						if qtty > 0 then
							...
							if isDead then
								...
								_TriggerServerEvent('esx_ambulancejob:removeItem', 'medikit')
								_TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(closestPlayer))
								IsBusy = false
					...
					ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(qtty)
						if qtty > 0 then
							...
							if health > 0 then
								...
								_TriggerServerEvent('esx_ambulancejob:removeItem', 'bandage')
								_TriggerServerEvent('esx_ambulancejob:heal', GetPlayerServerId(closestPlayer), 'small')
					...
					ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(qtty)
						if qtty > 0 then
							...
							if health > 0 then
								...
								_TriggerServerEvent('esx_ambulancejob:removeItem', 'medikit')
								_TriggerServerEvent('esx_ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
								ESX.ShowNotification(_U('heal_complete', GetPlayerName(closestPlayer)))
								IsBusy = false
							else
								ESX.ShowAdvancedNotification('Ambulance', 'Message', _U('player_not_conscious'), 'CHAR_CALL911', 1)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							_TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', 'Ambulance', amount)
