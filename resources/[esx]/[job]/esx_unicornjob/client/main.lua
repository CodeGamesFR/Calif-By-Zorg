	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('esx_society:removeVehicleFromGarage', 'unicorn', vehicleProps)
			end, function(data, menu)
			...
			if not DoesEntityExist(vehicle) then
				...
				if Config.MaxInService == -1 then
					ESX.Game.SpawnVehicle(model, {
						...
						_TriggerServerEvent('esx_vehiclelock:givekey', 'no', newPlate)
	...
	while true do
		if WarMenu.IsMenuOpened('unicornmenu') then
			...
			if WarMenu.Button("Carte d'itentité") then
				...
				if distance ~= -1 and distance <= 3.0 then
					_TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(player), GetPlayerServerId(PlayerId()))
				else
		...
		elseif WarMenu.IsMenuOpened('craftmenu') then
			if WarMenu.Button(_U('jagerbomb')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'jagerbomb')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('golem')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'golem')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('whiskycoca')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'whiskycoca')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('vodkaenergy')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'vodkaenergy')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('vodkafruit')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'vodkafruit')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('rhumfruit')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'rhumfruit')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('teqpaf')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'teqpaf')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('rhumcoca')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'rhumcoca')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('mojito')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'mojito')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('mixapero')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'mixapero')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('metreshooter')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'metreshooter')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end

			if WarMenu.Button(_U('jagercerbere')) then
				_TriggerServerEvent('esx_unicornjob:craftingCoktails', 'jagercerbere')
				animsAction({lib = "mini@drinking", anim = "shots_barman_b"})
			end
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
		...
		if player ~= -1 and distance <= 3.0 then
			...
			if amount == nil then
				...
				_TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_unicorn', _U('billing'), amount)
			end
		else
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('esx_unicornjob:getStockItem', itemName, count)
				end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('esx_unicornjob:putStockItems', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fridge_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'fridge_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('esx_unicornjob:getFridgeStockItem', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fridge_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'fridge_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('esx_unicornjob:putFridgeStockItems', itemName, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'unicorn_shop', {
		...
		_TriggerServerEvent('esx_unicornjob:buyItem', data.current.value, data.current.price, data.current.label)
	end, function(data, menu)
	end)
end
			...
			if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
				...
				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('esx_society:putVehicleInGarage', 'unicorn', vehicleProps)
					else
						if
							GetEntityModel(vehicle) == `rentalbus`
						then
							_TriggerServerEvent('esx_service:disableService', 'unicorn')
						end
					end
