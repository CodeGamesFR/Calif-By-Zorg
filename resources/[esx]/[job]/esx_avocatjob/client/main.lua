	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				_TriggerServerEvent('esx_society:removeVehicleFromGarage', 'avocat', vehicleProps)
			end, function(data, menu)
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount <= 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						_TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_avocat', 'Avocat', amount)
					end
				end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil or count <= 0 then
					...
					menu.close()
					OpenGetStocksMenu()
					_TriggerServerEvent('esx_avocatjob:getStockItem', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil or count <= 0 then
					...
					menu.close()
					OpenPutStocksMenu()
					_TriggerServerEvent('esx_avocatjob:putStockItems', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
			...
			if IsControlPressed(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'avocat' and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						_TriggerServerEvent('esx_society:putVehicleInGarage', 'avocat', vehicleProps)
