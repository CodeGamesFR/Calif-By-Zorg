		...
		elseif WarMenu.IsMenuOpened('citoyenmenu') then
			...
			if WarMenu.Button('Fouiller') then
				...
				if distance ~= -1 and distance <= 3.0 then
					...
					_TriggerServerEvent('esx_policejob:message', GetPlayerServerId(player), _U('being_searched'))
			...
			if WarMenu.Button('Escorter') then
				local player, distance = ESX.Game.GetClosestPlayer()
				if distance ~= -1 and distance <= 3.0 then
					_TriggerServerEvent('esx_policejob:drag', GetPlayerServerId(player))
				else
					ESX.ShowNotification('~r~Aucun joueur~s~ à proximité')
				end
			end
			...
			if WarMenu.Button('Mettre dans le véhicule') then
				local player, distance = ESX.Game.GetClosestPlayer()
				if distance ~= -1 and distance <= 3.0 then
					_TriggerServerEvent('esx_policejob:putInVehicle', GetPlayerServerId(player))
				else
					ESX.ShowNotification('~r~Aucun joueur~s~ à proximité')
				end
			end
			...
			if WarMenu.Button('Sortir du véhicule') then
				local player, distance = ESX.Game.GetClosestPlayer()
				if distance ~= -1 and distance <= 3.0 then
					_TriggerServerEvent('esx_policejob:OutVehicle', GetPlayerServerId(player))
				else
					ESX.ShowNotification('~r~Aucun joueur~s~ à proximité')
				end
			end
		...
		elseif WarMenu.IsMenuOpened('fouillermenu') then
			...
			for i = 1, #fouilleElements, 1 do
				if WarMenu.Button(fouilleElements[i].label) then
					_TriggerServerEvent('GangsBuilder:confiscatePlayerItem', GetPlayerServerId(searchedPly), fouilleElements[i].itemType, fouilleElements[i].value, fouilleElements[i].amount)
