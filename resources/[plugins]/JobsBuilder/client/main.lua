...
Citizen.CreateThread(function()
	...
	_TriggerServerEvent('JobsBuilder:requestSync')
	_TriggerServerEvent('JobsBuilder:requestBlips')
end)
						...
						if amount ~= nil and amount > 0 then
							...
							if player ~= -1 or distance <= 3.0 then
								...
								Citizen.CreateThread(function()
									...
									_TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_' .. ActualJob.Name, ActualJob.Label, amount)
								end)
							else
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('JobsBuilder:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('JobsBuilder:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('JobsBuilder:addArmoryWeapon', function()
			OpenPutWeaponMenu()
		end, data.current.value, data.current.ammo)
	end, function(data, menu)
	end)
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('JobsBuilder:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('JobsBuilder:getStockItem', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('JobsBuilder:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					_TriggerServerEvent('JobsBuilder:putStockItems', data.current.value, count)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
AddEventHandler('JobsBuilder:hasEnteredMarker', function(part)
...
AddEventHandler('JobsBuilder:hasExitedMarker', function(part)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
		...
		if PlayerData.job ~= nil and ActualJob then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastPart ~= currentPart)) then
				if (LastPart ~= nil) and (LastPart ~= currentPart) then
					TriggerEvent('JobsBuilder:hasExitedMarker', LastPart)
					hasExited = true
				end
				...
				TriggerEvent('JobsBuilder:hasEnteredMarker', currentPart)
			end
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0, 38) and PlayerData.job ~= nil and ActualJob and (GetGameTimer() - GUI.Time) > 150 then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					_TriggerServerEvent('esx_society:putVehicleInGarage', ActualJob.Name, vehicleProps)
				...
				if CurrentAction == 'menu_boss_actions' then
					...
					TriggerEvent('esx_society:openBossMenu', ActualJob.Name, function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, {wash = true})
