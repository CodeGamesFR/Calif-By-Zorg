...
function releaseHostage()
	...
	if closestDistance ~= -1 and closestDistance <= 3 then
		...
		_TriggerServerEvent('cmg3_animations:sync', lib, lib2, anim1, anim2, distans, distans2, height, target, length, spin, controlFlagMe, controlFlagTarget, animFlagTarget, attachFlag)
	end
end
...
RegisterCommand('porter', function(source, args)
	...
	if not piggyBackInProgress then
		...
		if closestDistance ~= -1 and closestDistance <= 3 then
			...
			_TriggerServerEvent('cmg2_animations:sync', lib, anim1, anim2, distans, distans2, height, target, length, spin, controlFlagMe, controlFlagTarget, animFlagTarget)
		end
	else
		...
		if closestDistance ~= -1 and closestDistance <= 3 then
			local target = GetPlayerServerId(closestPlayer)
			_TriggerServerEvent('cmg2_animations:stop', target)
		end
	end
end, false)
	...
	while true do
		if holdingHostage then
			...
			if IsEntityDead(plyPed) then
				...
				if closestDistance ~= -1 and closestDistance <= 3 then
					local target = GetPlayerServerId(closestPlayer)
					_TriggerServerEvent('cmg3_animations:stop', target)
				end
