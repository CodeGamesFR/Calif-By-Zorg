...
AddEventHandler('scrambler-vac:triggeredClientEvent', function(name, ...)
	...
	if xPlayer and name then
		...
		TriggerEvent('esx:customDiscordLog', ("Joueur : %s [%s] (%s) - Méthode : TriggerEvent(%s)"):format(xPlayer.name, xPlayer.source, xPlayer.identifier, eventName))
	end
end)
...
for i = 1, #events, 1 do
	...
	AddEventHandler(events[i], function(...)
		...
		if xPlayer and events[i] then
			...
			TriggerEvent('esx:customDiscordLog', ("Joueur : %s [%s] (%s) - Méthode : _TriggerServerEvent(%s)"):format(xPlayer.name, xPlayer.source, xPlayer.identifier, eventName))
		end
	end)
end
