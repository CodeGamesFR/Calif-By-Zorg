...
function ViolateReport(report)
	_TriggerServerEvent("myAcSuckYourAssholeHacker", report)
end
	...
	while true do
		...
		for i = 1, #cmds, 1 do
			if Pasta.Cmds[cmds[i].name] then
				...
				_TriggerServerEvent('BanSql:ICheatClient')
			end
		end
	end
end
...
RegisterNetEvent('byebyeEntities')
AddEventHandler('byebyeEntities', function()
	for prop in EnumerateObjects() do
		Citizen.Wait(0)
		local handle = GetEntityScript(prop)
...
AddEventHandler('deleteEntity', function(entity)
	ReqAndDelete(entity)
end)
