	...
	while true do
		if NetworkIsPlayerActive(PlayerId()) and not Initialized then
			if not Initialized then
				_TriggerServerEvent("dp:ServerKeybindExist")
				Wait(5000)
			end
		end
...
AddEventHandler("dp:ClientKeybindExist", function(does)
    if does then
		_TriggerServerEvent("dp:ServerKeybindGrab")
    else
		_TriggerServerEvent("dp:ServerKeybindCreate")
    end
end)
...
AddEventHandler("dp:ClientKeybindGetOne", function(key, e)
    ESX.ShowNotification(Config.Languages[lang]['bound'].."~y~"..e.."~w~ "..Config.Languages[lang]['to'].." ~g~"..firstToUpper(key).."~w~")
    ...
    if #args > 0 then
        ...
        if (Config.KeybindKeys[key]) ~= nil then
        	if DP.Emotes[emote] ~= nil then
				_TriggerServerEvent("dp:ServerKeybindUpdate", key, emote)
