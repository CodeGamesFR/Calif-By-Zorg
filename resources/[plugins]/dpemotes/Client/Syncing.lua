		...
		if #args > 0 then
			...
			if (distance ~= -1 and distance < 3) then
				if DP.Shared[emotename] ~= nil then
					...
					_TriggerServerEvent("ServerEmoteRequest", GetPlayerServerId(target), emotename)
	...
	while true do
		...
		if IsControlJustPressed(0, 246) and isRequestAnim then
			...
			if (distance ~= -1 and distance < 3) then
				...
				_TriggerServerEvent("ServerValidEmote", GetPlayerServerId(target), requestedemote, otheremote)
				isRequestAnim = false
			else
				ESX.ShowNotification(Config.Languages[lang]['nobodyclose'])
			end
