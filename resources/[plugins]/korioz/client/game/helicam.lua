...
RegisterNetEvent('korioz_vehicle:spotlight')
AddEventHandler('korioz_vehicle:spotlight', function(senderId, state)
	...
	while true do
		...
		if IsVehicleModel(plyVeh, polmav_hash) then
			...
			if IsControlJustPressed(0, 183) and GetPedInVehicleSeat(plyVeh, -1) == plyPed then
				...
				_TriggerServerEvent("korioz_vehicle:spotlight", spotlight_state)
